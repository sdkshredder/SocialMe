...
class MessagesVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("messageCell") as! MessageTVCell
        let info : (String, [String]) = data[indexPath.row] as (String, [String])
        
        cell.senderLabel.text = info.0
        // let messages_info : [String] = info.1
        cell.messageLabel.text = "message info" // messages_info[messages_info.count - 1].0
